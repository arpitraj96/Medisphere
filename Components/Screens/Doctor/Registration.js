...
const Registration = () => {
  ...
  const [BadYourName, setBadYourName] = useState('')
  ...
  const validate=()=>{
    ...
    {if(YourName==''){
      validYourName = false;
      setBadYourName('Please Enter Name');
    }else if(YourName!='' && YourName.length<=3){
      validYourName = false;
      setBadYourName('Please Enter Valid Name');
    }else if(YourName!='' && YourName.length>3 && !YourName.toString().match(nameRegex)){
      validYourName = false;
      setBadYourName('Please Enter Valid Name');
    }else if(YourName!='' && YourName.length>3 && YourName.toString().match(nameRegex)){
      validYourName = true;
    ...
    return validYourName && validHighestQualification && validSpeciality && validProfessionalBio && validSpecialisedTreatment && validIndustryExperience &&validClinicAddress
  }
  ...
  const registerUser=()=>{
    ...
    setLoading(true)
    firestore().collection("RegisteredDoctor").add({
      Email, YourName, HighestQualification, Speciality, ProfessionalBio, SpecialisedTreatment, IndustryExperience, ClinicAddress, AwardsPublications: awardsPublications.map(entry => entry.value),
    }).then(()=>{
      setYourName(''); setHighestQualification(''); setSpeciality(''); setProfessionalBio(''); setSpecialisedTreatment(''); setIndustryExperience(''); setClinicAddress(''); setAwardsPublications([{ id: 1, value: '' }]);
      setDoneRegistration(true);
      setLoading(false);
      mainPage(YourName, HighestQualification, Speciality, ProfessionalBio, SpecialisedTreatment, IndustryExperience, ClinicAddress)
  ...
  const mainPage = async(name,qualification,speciality,bio,specialisedTreatment,industryExperience,clinicAddress) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      {!DoneRegistration?(<ScrollView>
        ...
        <View>
          <CustomTextInput style={styles.name} value={YourName} onChangeText={txt => {setYourName(txt);}} 
          title="Your Name" placeholder={'Nidhi'} bad={BadYourName != ''?true:false}/>
          {BadYourName != '' && <Text style={styles.errorMsg}>{BadYourName}</Text>}
      ...
      />
        <View>
        <CustomSolidBtn title={'Register'} onClick={()=>{
          if(validate()){
            registerUser()
          }
        }}/>
